...
function normalizePersonSurface(name) {
  return name.normalize("NFC").trim();
}
function segmentPersonName(fullName, surnames) {
...
function romanizeSplitParts(split, romanize, fallbackName) {
  const parts = [split.familyName, split.givenName].map((part) => romanize(part)).filter((value) => typeof value === "string" && value.trim());
