...
<DATA_VERSION_TAG>`) into `GROGNARD_PLUGIN_CACHE_PATH`
...
`_p5.rng` / `_p5.sch` with the Grognard loosenings). Without a prebuilt index the
...
| JSON bridge (`status`/`sync`/`install_from_source`/`search`/`resolve`/`convert`) | `python/cbeta_import/grognard_bridge.py` | done |
...
python3 scripts/build-cbeta-metadata.py \
  ...
  --corpus            "$GROGNARD_PLUGIN_CACHE_PATH/corpus/xml-p5" \
...
1. ~~`xml:id` collision~~ — resolved. CBETA-internal (multi-file) handled by
   `prefix_ids`. Grognard-vs-CBETA: Grognard's editor ids are `dom_N`
...
4. ~~Sanity-check `downgrade.phonetic_glosses` against a real 音義 juan~~ —
   done against `T54n2128` (慧琳 一切經音義, in the synced 2026R1 corpus). It
   **does** use `cb:yin`, but only nested in `cb:fan` and as *bare text*
   (`<cb:fan><cb:zi>字</cb:zi><cb:yin>某某反</cb:yin></cb:fan>`) — no `cb:sg`,
   no `<note>`. The synthetic fixture only covered the `cb:sg` / `<note>`
   shapes, so all 5 fanqie readings in that juan were silently dropped. Fixed:
   the `cb:fan` branch now falls back to bare-text `cb:yin`
   (`fanqie=True`); `test_fan_bare_text_yin` / `test_fan_yin_with_interrupting_lb`
   are the regression from the real data.

### Maintainer note — bundled metadata refresh

The recipe for rebuilding `data/metadata/*` on a new CBETA release (which DILA
repos, the DILA→Norbert crosswalk extraction from the authority-extraction
Norbert `concordance.ndjson`, the `gh api …/trees` file-list, `--corpus` vs
`--file-list`) is **[docs/metadata-refresh.md](docs/metadata-refresh.md)**.
